...
original_author="orhant"
original_name="py_srver2"
original_urlname="py-srver2"
original_description="Awesome py_srver2 created by orhant" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/py_srver2 "${name}"
...
then
    ...
    echo "then run 'py_srver2' or 'python -m py_srver2'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'py_srver2' or 'python -m py_srver2' or 'poetry run py_srver2'"
fi
